...
#define SYS__202000173_ADD_MEMORY_LIMIT 557
#define SYS__202000173_GET_MEMORY_LIMITS 558
#define SYS__202000173_UPDATE_MEMORY_LIMIT 559
#define SYS__202000173_REMOVE_MEMORY_LIMIT 560
...
void add_memory_limit(pid_t pid, size_t memory_limit) {
	...
	if (syscall(SYS__202000173_ADD_MEMORY_LIMIT, pid, memory_limit) < 0) {  //FIXME != 0
		perror("SYS__202000173_ADD_MEMORY_LIMIT");
		return;
	}
...
void get_memory_limits(size_t max_entries) {
	...
	if (syscall(SYS__202000173_GET_MEMORY_LIMITS, buffer, max_entries, &processes_returned) < 0) {  //FIXME != 0
		perror("SYS__202000173_GET_MEMORY_LIMITS");
		free(buffer);
		return;
	}
...
void update_memory_limit(pid_t pid, size_t memory_limit) {
	if (syscall(SYS__202000173_UPDATE_MEMORY_LIMIT, pid,  memory_limit) < 0) {  //FIXME != 0
		perror("SYS__202000173_UPDATE_MEMORY_LIMIT");
		return;
	}
...
void remove_memory_limit(pid_t pid) {
	if (syscall(SYS__202000173_REMOVE_MEMORY_LIMIT, pid) < 0) {  //FIXME != 0
		perror("SYS__202000173_REMOVE_MEMORY_LIMIT");
		return;
	}
